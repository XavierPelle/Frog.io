<main>
    <div class="container">
        ...
        <form action="index.php?page=especes&action=<?php echo $action;
                                                    echo (isset($id)) ? '&id=' . $id : ''; ?>" method="POST" enctype="multipart/form-data">
            <?php if (isset($action) && $action === 'createH') { ?>
                <div class="mb-3">
                    <label for="parent1" class="form-label">Parent 1</label>
                    <select class="form-select" name="parent1">
                        <?php foreach ($especeALL as $especeOne) {
                            echo "<option value='$especeOne->id'>$especeOne->nomScientifique</option>";
                        }
                        ?>
                    </select>
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-select" name="parent2">
                        <?php foreach ($especeALL as $especeOne) {
                            echo "<option value='$especeOne->id'>$especeOne->nomScientifique</option>";
                        }
                        ?>
